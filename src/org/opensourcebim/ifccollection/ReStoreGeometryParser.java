 ...
 *
 */
public class ReStoreGeometryParser {
	...
	public ReStoreGeometryParser(IfcModelInterface ifcModel) {
	 ...
	 */
	public ReStoreGeometry getGeometryFromProduct(IfcProduct prod) {
		...
		if (geometry != null) {
			...
			try {
				...
				if (geomData != null && geomData.size() > 0) {
					...
					geom.setLargestFaceArea(largest_face_area);
	...
	private Double convertVolume(Double value) {
		return ReStoreGeometryParser.getVolumeUnit().convert(value, modelVolumeUnit);
	}

	private Double convertArea(Double value) {
		return ReStoreGeometryParser.getAreaUnit().convert(value, modelAreaUnit);
	}

	private Double convertLength(Double value) {
		return ReStoreGeometryParser.lengthUnit.convert(value, modelLengthUnit);
	}
	 ...
	 */
	public void tryParseFloorArea(IfcModelInterface ifcModel, ObjectStoreImpl objectStore, byte[] data) {
		...
		if (allSpaces.size() > 0) {
			for (IfcSpace space : allSpaces) {
				...
				ReStoreGeometry geom = getGeometryFromProduct(space);

				// ToDo: also include geometric check?
				if (!isIncludedSemantically) {
					objectStore.getSpaces()
							.add(new ReStoreSpaceImpl(space.getGlobalId(), geom.getVolume(), geom.getFloorArea()));
				}
			}
		} else if (data != null){
			JsonNode res;
			...
			objectStore.getSpaces()
			.add(new ReStoreSpaceImpl("no floor area found", 0.0, -1));
		}
