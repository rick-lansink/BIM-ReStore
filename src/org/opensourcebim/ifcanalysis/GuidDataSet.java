...
public class GuidDataSet {
	...
	public GuidDataSet(ObjectStore store) {
		...
		for (ReStoreObject obj : store.getObjects()) {
			...
			this.setRecordValue(guid,  "dimensions", obj.getGeometry().getDimensions());
			...
			this.setRecordValue(guid, "properties", obj.getProperties());
			...
			if (!obj.getParentId().isEmpty()) {
				Optional<ReStoreObject> parent = store.getObjectByGuid(obj.getParentId());
				if (parent.isPresent()) {
